...
import { ForgotPasswordDto, LoginAuthDto, RegisterUserDto, ResetPasswordDto, UpdateMeDto } from '~/dtos/auth.dto'
import { UsersCollection, UsersSchema } from '~/models/users.schema'
...
class AuthService {
  ...
  async logout({ user_id, refresh_token }: { user_id: string; refresh_token: string }) {
    ...
    //
    return await userTokensService.deleteByToken({ refresh_token }) // Chỉ đăng xuất trên phiên hiện tại
  }
