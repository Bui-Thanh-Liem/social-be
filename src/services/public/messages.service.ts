...
import { IMessage } from '~/shared/interfaces/public/message.interface'
import { MessagesCollection, MessagesSchema } from '~/schemas/public/message.schema'
import { COLLECTION_USERS_NAME } from '~/schemas/public/user.schema'
import { IQuery } from '~/shared/interfaces/common/query.interface'
import { ResMultiType } from '~/shared/types/response.type'
import { getPaginationAndSafeQuery } from '~/utils/get-pagination-and-safe-query.util'
import { CreateMessageDto } from '../../shared/dtos/public/messages.dto'
...
class MessagesService {
  async create(sender_id: string, payload: CreateMessageDto) {
    ...
    if (payload.attachments) {
      _attachments = await uploadsService.getMultiByKeys(payload.attachments.map((a) => a.s3_key))
    }
  ...
  async deleteConversationMessages(conversationId: string) {
    ...
    for (const msg of messages) {
      ...
      try {
        await uploadsService.delete({ s3_keys: msg.attachments?.map((att) => att.s3_key) || [] })
      } catch (err) {
