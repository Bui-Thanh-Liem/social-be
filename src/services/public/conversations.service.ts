...
import { CreateConversationDto } from '~/shared/dtos/public/conversations.dto'
import { EConversationType } from '~/shared/enums/public/conversations.enum'
import { ENotificationType } from '~/shared/enums/public/notifications.enum'
import cacheService from '~/helpers/cache.helper'
import { notificationQueue, systemQueue } from '~/infra/queues'
import { IConversation } from '~/shared/interfaces/public/conversation.interface'
import { ConversationsCollection, ConversationsSchema } from '~/schemas/public/conversation.schema'
import { COLLECTION_MESSAGES_NAME } from '~/schemas/public/message.schema'
import { COLLECTION_USERS_NAME, UsersCollection } from '~/schemas/public/user.schema'
...
class ConversationsService {
  ...
  async getOneById(id: string, user_id: string) {
    ...
    await this.updateConversionDeleted(id)
  ...
  async read({ user_id, id }: { id: string; user_id: string }) {
    //
    const updated = await ConversationsCollection.findOneAndUpdate(
      { _id: new ObjectId(id) },
  ...
  async delete({ user_id, id }: { id: string; user_id: string }) {
    ...
    try {
      ...
      if (shouldDeleteCompletely) {
        ...
        await systemQueue.add(CONSTANT_JOB.DELETE_MESSAGES, { conversation_id: id })
        await ConversationsCollection.deleteOne({ _id: new ObjectId(id) })
      }
  ...
  async togglePinConversion({ user_id, id }: { user_id: string; id: string }): Promise<'Ghim' | 'Gỡ ghim'> {
  ...
  async addParticipants({ id, user_id, participants }: { user_id: string; id: string; participants: string[] }) {
    //
    const conv = await ConversationsCollection.findOne({ _id: new ObjectId(id) })
    if (!conv) throw new Error('Không tìm thấy cuộc trò chuyện')
  ...
  async removeParticipants({
    ...
    if (updated.participants?.length === 0) {
      await MessagesService.deleteConversationMessages(id)
      await ConversationsCollection.deleteOne({ _id: new ObjectId(id) })
    }
  ...
  async promoteMentor({ id, user_id, participant }: { user_id: string; id: string; participant: string }) {
    const conv = await ConversationsCollection.findOne({ _id: new ObjectId(id) })
    if (!conv) throw new NotFoundError('Không tìm thấy cuộc trò chuyện')
  ...
  private async updateConversionDeleted(id: string) {
    await ConversationsCollection.updateOne({ _id: new ObjectId(id) }, { $set: { deleted_for: [] } })
  }
