...
import { COLLECTION_BOOKMARKS_NAME } from '~/models/public/bookmarks.schema'
import {
...
import { COLLECTION_USERS_NAME, UsersCollection, UsersSchema } from '~/models/public/users.schema'
import { IQuery } from '~/shared/interfaces/query.interface'
import { ResMultiType } from '~/shared/types/response.type'
import { getPaginationAndSafeQuery } from '~/utils/get-pagination-and-safe-query.util'
import { slug } from '~/utils/slug.util'
import { ResSearchPending } from '../../dtos/public/search.dto'
import { EVisibilityType } from '../../enums/public/communities.enum'
import { ETweetAudience, ETweetType } from '../../enums/public/tweets.enum'
import { ITrending } from '../../interfaces/public/trending.interface'
import { COLLECTION_TWEETS_NAME, TweetsCollection, TweetsSchema } from '../../models/public/tweets.schema'
import CommunitiesService from './communities.service'
import followsService from './follows.service'
import TrendingService from './trending.service'
import tweetsService from './tweets.service'
import { COLLECTION_LIKES_NAME } from '~/models/public/likes.schema'
...
class SearchService {
  ...
  async searchPending({ query }: { query: IQuery<ITrending> }): Promise<ResSearchPending> {
    ...
    return {
      ...
      communities: CommunitiesService.signedCloudfrontCoverUrls(communities) as ICommunity[]
    }
  }
  ...
  async searchUser({ query, user_id }: { query: IQuery<IUser>; user_id: string }): Promise<ResMultiType<IUser>> {
    ...
    let users = await UsersCollection.aggregate<UsersSchema>(basePipeline).toArray()
    ...
    if (users.length === 0 && q) {
      ...
      users = await UsersCollection.aggregate<UsersSchema>(textPipeline).toArray()
    }
