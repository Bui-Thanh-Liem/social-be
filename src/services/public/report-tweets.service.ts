...
import { ReportTweetCollection } from '~/schemas/public/report-tweet.schema'
import { TweetsCollection } from '~/schemas/public/tweet.schema'
...
class ReportTweetService {
  async report({ reporter_id, tweet_id }: { tweet_id: string; reporter_id: string }) {
    ...
    const tweet = await TweetsCollection.findOne({ _id: tweetObjectId }, { projection: { user_id: 1 } })
